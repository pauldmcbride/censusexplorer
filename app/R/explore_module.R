...
explore_server <- function(id) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    pal_sa2 <- eventReactive(var(), {
      palette_maker(domain = sa2[[var()]])
    })

    current_ta <- reactiveVal("none")
    current_sa2 <- reactiveVal(NULL)
    ...
    observeEvent(list(var(),add_layer()), {
      ...
      if(is_empty(add_layer())) {
        ...
        sa2_in <- add_layer() %>%
          filter(!SA22023_V1 %in% current_sa2())
        current_sa2(c(current_sa2(), sa2_in$SA22023_V1))
        sa2_opacity <- main_opacity
      }
      ta_trim_ <- ta[!ta$TALB2023_1 %in% add_layer()$ta,]
      if(!identical(current_ta(),proxy_ta()) &&
         map_zoom_() >= 10) {
